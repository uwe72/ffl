...
public class ManagerController {
    ...
    @PreAuthorize("hasRole('ADMIN')")
    public ResponseEntity<?> updateManagerDetails(@PathVariable Long id, @Valid @RequestBody UpdateManagerDetailsRequest request) {
        ManagerDto updated = managerService.updateManagerDetails(id, request);
        if (updated == null) {
            return ResponseEntity.notFound().build();
        }
