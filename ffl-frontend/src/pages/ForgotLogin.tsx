...
export default function ForgotLogin() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    setIsLoading(true)
    try {
      await authApi.forgotLogin(email)
      setSuccess(true)
    } catch {
      setSuccess(true)
    } finally {
    ...
    <div className="relative min-h-screen flex items-center justify-center py-12 px-4 sm:px-6 lg:px-8 overflow-hidden">
      ...
      <div className="relative bg-surface/70 backdrop-blur-md border border-border rounded-card w-full max-w-[440px] max-h-[90vh] flex flex-col shadow-2xl ffl-login-enter">
        <div className="flex flex-col items-center text-center gap-1 px-6 pt-8 pb-2">
          <h2 className="text-2xl font-bold text-foreground leading-tight">Login vergessen</h2>
        ...
        <div className="flex-1 overflow-y-auto px-6 pb-6">
          {success ? (
            <div className="space-y-4 mt-2">
              <div className="flex items-center gap-3 p-3 bg-success-bg border border-success/30 rounded-control">
                ...
                <p className="text-success text-sm">
                  Falls ein Konto mit dieser E-Mail-Adresse existiert, wurden die Login-Namen an diese Adresse gesendet.
                </p>
              </div>
              <p className="text-muted text-sm">
            ...
            <form className="space-y-4 mt-2" onSubmit={handleSubmit} noValidate>
              <p className="text-muted text-sm">
                Gib deine E-Mail-Adresse ein und wir senden dir die damit verknüpften Login-Namen zu.
              </p>
