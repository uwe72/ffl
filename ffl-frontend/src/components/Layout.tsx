...
export default function Layout() {
  ...
  return (
    <div className="min-h-screen bg-[#0f1419]" style={{ backgroundImage: `url('/${isAuthenticated ? 'after_login' : 'background'}.png')`, backgroundSize: 'cover', backgroundPosition: 'center', backgroundAttachment: 'fixed' }}>
          ...
          <div className="flex items-center justify-between">
            ...
            <nav className="hidden md:flex gap-6 items-center">
              {isAuthenticated && user?.role === 'ADMIN' && (
                <>
                  ...
                  <RouterLink to="/emails" className="text-[#a0aec0] hover:text-[#c9a66b] link transition-colors">
                    E-Mails
                  </RouterLink>
              ...
              <a href="/feedback" target="_blank" rel="noopener noreferrer" className="text-[#a0aec0] hover:text-[#c9a66b] link transition-colors">
                Feedback
              </a>
            </nav>
            <div className="flex flex-col items-end">
              <div className="flex items-center gap-2 md:gap-4">
                ...
                {isAuthenticated ? (
                  <>
                    <RouterLink to="/profile" className="hidden md:inline-flex items-center gap-1.5 px-3 py-1 rounded-lg bg-[#242d38] text-sm text-[#c9a66b] hover:bg-[#2d3748] hover:text-[#d4b77a] transition-colors border border-[#3d4a5c]">
                      <span>{user?.login}</span>
                      <span className="text-[#6b7280] text-xs">({user?.role === 'ADMIN' ? 'Admin' : 'User'})</span>
                    </RouterLink>
                    <RouterLink to="/profile" className="md:hidden inline-flex items-center px-2 py-1 rounded-lg bg-[#242d38] text-xs text-[#c9a66b] hover:bg-[#2d3748] transition-colors border border-[#3d4a5c]">
                      {user?.login}
                    </RouterLink>
